...
$jumlahData = count(query("SELECT judul, tanggal, nama, nim, foto, topik.id_topik, id_user FROM topik INNER JOIN anggota ON topik.id_user = anggota.id_anggota WHERE judul LIKE '%$keyword%' ORDER BY tanggal DESC"));
$jumlahHalaman = ceil($jumlahData / $jumlahDataPerhalaman);
$halamanAktif = (isset($_GET['halaman']) ? $_GET['halaman'] : 1);
$awalData = ($jumlahDataPerhalaman * $halamanAktif) - $jumlahDataPerhalaman;
$forum = query("SELECT judul, tanggal, nama, nim, foto, topik.id_topik, id_user FROM topik INNER JOIN anggota ON topik.id_user = anggota.id_anggota WHERE judul LIKE '%$keyword%' ORDER BY tanggal DESC LIMIT $awalData, $jumlahDataPerhalaman");
// End
?>
...
<section class="" style="background-color: #FAF7F5">
    ...
    <div class="container p-3 mx-auto ">
        <h1 class="text-header font-bold text-3xl text-center mt-4 mb-6">Forum Diskusi</h1>
            ...
            <div class="search">
                <div class="relative">
                    ...
                    <input type="text" id="default-search" name="keyword"
                        ...
                        placeholder="Cari berdasarkan judul..." value="<?php echo $keyword; ?>">
        ...
        <h4 class="text-header font-bold text-lg mt-4 mb-4 font-body">Daftar Topik :</h4>
        <hr class="w-48 h-1 mx-auto my-4 bg-gray-300 border-0 rounded md:my-10">
        <div class="row mx-auto">
            <?php if (!empty($forum) > 0) { ?>
                <?php foreach ($forum as $data): ?>
                    <div class=" mt-3 p-6 bg-white border border-gray-200 rounded-lg shadow-lg">
                        <div class="flex items-center gap-4">
                            <img class="w-12 h-12 p-1 rounded-full ring-2 ring-gray-300"
                                src="../assets/profile/<?php echo ($data['foto']) ?>" alt="">
                            <div class="font-medium font-body">
                                <span class="text-secondary">Dari
                        ...
                        <a href="topic.php?id_topik=<?php echo $data['id_topik']; ?>">
                            <h5 class="mb-2 text-xl mt-3 font-semibold font-header tracking-wider underline"
                                style="color: #00B5FF; text-decoration-color: #00B5FF;">
                                <?php echo htmlentities($data['judul']); ?>
                            </h5>
                        </a>

                        <?php if ($_SESSION['login']['id_anggota'] == $data['id_user']) { ?>
                            <div class="flex justify-end gap-1">
                                <a href="edit.php?id_topik=<?php echo $data['id_topik']; ?>"
                                ...
                                <button data-modal-target="<?php echo $data['id_topik']; ?>"
                                    data-modal-toggle="<?php echo $data['id_topik']; ?>"
                                    class="text-header font-medium rounded-full text-sm px-5 py-2.5 text-center transition-all duration-100 ease-in-out transform active:scale-90"
                                ...
                                <div class="relative p-4 w-full max-w-md max-h-full">
                                    <div class="relative bg-white rounded-lg shadow">
                                        ...
                                        <div class="p-4 md:p-5 text-center">
                                            ...
                                            <button data-modal-hide="<?php echo $data['id_topik']; ?>" type="button"
                                                ...
                                                <a href="delete_topic.php?id_topik=<?php echo $data["id_topik"] ?>"
